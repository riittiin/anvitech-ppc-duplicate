...
**One row of that table changes under the CP engine — see the next section.**

---

## 8. If the app is switched to the CP engine (`DEFAULT_SCHEDULER=cp`)

Nothing here applies until someone sets that environment variable on Render.
Once it is set, four things about this box become **operational requirements**,
not nice-to-haves.

**a) The box needs the solver installed.** The CP engine solves with a library
called `pyjobshop`, which is deliberately kept off the app's own server (Render
only ever *replays* a finished plan, and never solves). The setup script in
step 4 installs it. **If you built this box before 2026-08-14, SSH in and run
one line:**
...
Without it, every deep search fails on this box.

**b) 4 CPUs is not optional — and the free tier already gives you exactly
that.** Step 2 tells you to create the VM at **4 OCPU**, which is the maximum
the Always Free tier allows and also the minimum this engine wants. The reason
is measured, not theoretical: going from 2 to 4 solver threads at the *same* 30
minutes improved the *proven floor* on late-days from 170 to 215 — about 26%
better — while running the same solve for six times as long at 2 threads
improved it by two days. **More CPUs buy a better guarantee; more time only
buys a slightly better plan.** A box with fewer CPUs does not error. It just
returns a weaker answer and says nothing, which is why this is written down.

**c) There is NO local fallback under CP.** The last row of the failure table
above — "box down → GitHub tier → local watchdog, the button never dies" — is
only two-thirds true here. Oracle → GitHub still works (the GitHub runner
installs the solver too). But the final "compute it on Render instead" tier
**cannot** happen, because Render has no solver and never will. So if both this
box and GitHub are unavailable, a deep search reports **failed** rather than
running slowly. The plan already on screen is untouched and the shop keeps
working; only the *search* is unavailable. Press it again when a worker is back.

**d) The result is a good plan with a floor under it — never "the optimal
plan".** At this shop's size the solver reliably returns a complete, rule-legal
schedule **plus** something the old engine never had: a proven statement that
*no* schedule of this book could beat N late-days. What it does **not** return
is proof that its own plan is the best one; measured, its answer sits about
twice its own floor, and that gap does not close with more time. If anyone
writes "provably optimal" into a report or a screen, that is wrong.

**Before switching it on**, run the cheap pre-flight against the real book:
`scripts/cp_tractability_spike.py --real --mode variants`. Every number quoted
above was measured on a stand-in book with 14 machines against the shop's real
~26, whose delivery dates had to be pulled ten days earlier before it was even
late — so it is a proxy, and this one command re-checks the whole comparison
against the actual order book without a long solve.
